...
class Classification:
    ...
    def predict(self, X_test, y_test):
        ...
        for l in range(self.n):
            ...
            self.F1_score.append(f1_score(y_test, yhat))
...
democrats_12_1 = np.round(np.count_nonzero(y_train)/len(y_train),decimals=2)
democrats_12_2 = np.round(np.count_nonzero(y_train)/len(y_train),decimals=2)
democrats_12 = (democrats_12_1 + democrats_12_2) / 2

# 2016 US election
X, y = import_election_data(2016)
democrats_16 = np.round(np.count_nonzero(y)/len(y),decimals=2) # % of democrats
...
SVM_2.plot_accuracy(SVM_2.F1_score, 'F1 score as a function of different C indices. Testing on the 2016 election.')
# 2016 US election
SVM_2.predict(X, y)
